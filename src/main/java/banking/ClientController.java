...
public class ClientController {
    ...
    public void show() {
        ...
        Label infoLabel = new Label(
                "User: " + client.getUsername()
                        + " | Acc: " + client.getAccount().getAccountNumber()
        );
        ...
        depBtn.setOnAction(e -> {
            try {
                ...
                if (client.getAccount().deposit(amt)) {
                    show();
                } else {
                    new Alert(
                            Alert.AlertType.ERROR,
                            "Deposit failed. Ensure you are Verified."
                    ).showAndWait();
                }
        ...
        withBtn.setOnAction(e -> {
            try {
                double amt = Double.parseDouble(amountInput.getText());
                if (client.getAccount().withdraw(amt)) {
                    show();
                } else {
                    new Alert(
                            Alert.AlertType.ERROR,
                            "Withdraw failed. Check balance and status."
                    ).showAndWait();
                }
            } catch (NumberFormatException ex) {
                new Alert(
    ...
    private void openTransferDialog() {
        ...
        ButtonType sendBtn = new ButtonType(
                "Send Request",
        ...
        grid.add(new Label("Recipient Username:"), 0, 0);
        grid.add(toUser, 1, 0);
        grid.add(new Label("Recipient Account #:"), 0, 1);
        grid.add(toAcc, 1, 1);
            ...
            if (btn == sendBtn) {
                try {
                    ...
                    if (client.getAccount().getBalance() < amt) {
                        new Alert(
                                Alert.AlertType.ERROR,
                                "Insufficient funds to request this transfer!"
                        ).showAndWait();
                        return null;
                    }

                    User recipient = FakeDatabase.findUser(
                            toUser.getText(),
                            toAcc.getText()
                    );

                    if (recipient != null
                            && recipient.getAccount().getStatus().equals("Verified")) {
        ...
        result.ifPresent(req -> {
            ...
            new Alert(
                    ...
                    "Request submitted to Admin for approval."
            ).showAndWait();
        });
    }
}
